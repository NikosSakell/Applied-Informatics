...
class DynamicSieve {
    ...
    private final boolean[] prime; // Now using a boolean array
    ...
    public DynamicSieve(int size) {
        ...
        for (int i = 2; i <= size; i++) { // Initialize all as prime
            prime[i] = true;
        }
    }

    public void calculatePrimes() {
        int totalTasks = size - 1; // Excluding 0 and 1
        SieveWorker[] myThreads = new SieveWorker[numThreads];

        long startTime = System.currentTimeMillis();

        // Create and start workers
        for (int i = 0; i < numThreads; i++) {
            myThreads[i] = new SieveWorker(totalTasks, prime, lock);
            myThreads[i].start();
        }
    ...
    private static int getTask(int totalTasks, ReentrantLock lock) {
        ...
        try {
            if (++tasksAssigned < totalTasks) {
                return tasksAssigned + 2; // Start assignments at number 2
            } else {
                return 0; // Signal end of tasks
            }
        } finally {
            lock.unlock();
        }
    ...
    class SieveWorker extends Thread {
        ...
        public SieveWorker(int totalTasks, boolean[] prime, ReentrantLock lock) {
